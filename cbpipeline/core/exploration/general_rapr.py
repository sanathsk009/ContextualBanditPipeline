...
from cbpipeline.core.arm_elimination.arm_elimination import ArmEliminator
...
class GenRAPR:
    ...
    def __init__(
        ...
        arm_eliminator=ArmEliminator(),
        is_elim=False,
        elim_fraction=0.95,
        sequential_elim_fraction=0.6,
        error_data_ratio=0.25,
        calculate_alpha_using_eta=False,
    ) -> None:
        # model parameters
        self.model: Dict[Hashable, Any] = {}
        self.regressor = regressor
        self.error_estimator = error_estimator
        self.arm_eliminator = arm_eliminator
        ...
        self._excess_risk = 1
        self._optimal_cover_upper_bound: float = 1.0  # Um
        ...
        self.calculate_alpha_using_eta = calculate_alpha_using_eta
        ...
        # Out of sample (deploying learnt policy at the end of experiment)
        self.is_out_of_sample = False
    ...
    def _update(self) -> None:
        ...
        error_estimator = copy.deepcopy(self.error_estimator)
        error_estimator.fit(X_error, A_error, Y_error, P_error, Y_hat_error)
        self._excess_risk = error_estimator.get_error()

        errortrain_end_time = time.time()
        self.tracker["update_error_time"].append(
            errortrain_end_time - modeltrain_end_time
        )
        self.tracker["error"].append(np.sqrt(self._excess_risk))

        # Calculate risk adjustment and optimal cover.
        self._calculate_risk_adj()
        self._calculate_alpha()
        self.tracker["alpha"].append(self._alpha)
        ...
        if self.is_elim:
            self.arm_eliminator.fit(
                ...
                self._P,
                Y_hat_all,
                np.sqrt(self._excess_risk),
                self._alpha,
                self.model,
                actions,
                # training_data=(X_train, A_train, Y_train, P_train),
        ...
        self._optimal_cover_upper_bound = np.sqrt(self._alpha * self._excess_risk)
        self._expected_diff = self._risk_adj * self._optimal_cover_upper_bound
    ...
    def _get_most_risky_CASs(
        ...
        expected_diff = risk_adj * self._optimal_cover_upper_bound
    ...
    def _calculate_alpha(self):
        """_summary_
        Calculate alpha (the bound on the optimal cover).
        The loose theoretical bound primarily relies on avg size of most risky conformal arm set (beta=beta_max).
        Since theoretical bounds are loose, we heurestically ignore the terms that are not avg_set_size.
        """
        K = len(self._actions)
        if self.calculate_alpha_using_eta:
            self._alpha = K / self._risk_adj
        else:
            # First need predicted outcomes on observed data.
            # Initialize outcome estimates
            actions = self._actions
            ...
            # Get average size of most risky conformal arm sets
            above_threshold = self._get_most_risky_CASs(Y_hat=Y_hat, Y_greedy=Y_greedy)

            # Restrict attention to surviving arms if arm elimination.
            if self.is_elim:
                s_arms = np.ones((len(self._X), K))
                for elim in self.arm_eliminator_list:
                    s_arms = s_arms * elim.predict(self._X, actions)
                above_threshold = above_threshold * s_arms
                above_threshold = np.maximum(above_threshold, Y_greedy - Y_hat == 0)

            setsize_for_prior_contexts = np.sum(above_threshold, 1)
            avg_set_size = np.sum(setsize_for_prior_contexts) / len(
                setsize_for_prior_contexts
            ) + K * np.sqrt(1 / len(setsize_for_prior_contexts))
            avg_set_size = min(K, avg_set_size)

            self._alpha = avg_set_size
    ...
    def predict(self, context, actions, is_batch=False):
        ...
        # else can use past data to construct probability over arms.
            ...
            self._expected_diff = self._optimal_cover_upper_bound * self._risk_adj
            beta_thresholds = np.minimum(
                self._expected_diff
            ...
            # print("self._expected_diff", self._expected_diff)
            ...
            #     np.maximum(theta_max - theta_hat, self._expected_diff),
            # )
            # print("beta_thresholds", beta_thresholds)
            # print("sorted_beta_thresholds", sorted_beta_thresholds)
            # print(
            #     "surviving_arms",
            #     surviving_arms,
            #     np.sum(np.sum(surviving_arms, axis=1) == 0),
            # )
            ...
            for beta in sorted_beta_thresholds.T:
                ...
                candidate_probs = (
                    ...
                    * (arm_set)
                    ...
                    + (1 - self.elim_fraction) * conformal_probs
                )
